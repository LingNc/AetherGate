这是一个针对 AetherGate v1.12 的功能增强说明书。

更新内容：

新增配置项：传送落点搜索半径 (teleport.arrival-radius)。允许自定义到达祭坛时寻找安全落点的范围（默认 2，即 5x5 区域）。

新增配置项：炸膛爆炸威力 (altar.backfire-power)。允许自定义祭坛结构错误或被破坏时的爆炸威力（默认 6.0）。

请将以下文档交付给开发人员。

AetherGate 功能增强说明书 (v1.12)
优先级: 低 (Configuration) 目标版本: Paper 1.21.4

1. 配置文件更新 (config.yml)
涉及文件: plugin/src/main/resources/config.yml

修改: 添加新的配置项。
# AetherGate Configuration
...
altar:
  ...
  # 祭坛炸膛时的爆炸威力 (0.0 为无伤害)
  # 注意：此爆炸默认不破坏方块 (blockDamage=false)，但会伤害实体
  backfire-power: 6.0

teleport:
  # 抵达目标祭坛时，寻找安全落点的半径
  # 默认 2 (即以核心为中心 -2 到 +2 的 5x5 范围)
  arrival-radius: 2
...
修改: 在 backfire 方法中应用配置的爆炸威力。

4. 传送服务更新 (TeleportService.java)
涉及文件: plugin/src/main/java/cn/lingnc/aethergate/teleport/TeleportService.java

修改: findArrivalSpot 方法现在使用配置中的半径。
...
public class TeleportService {
    ...
    private static final int PREVIEW_TICKS = 10;
    ...
    private Location findArrivalSpot(Waypoint destination) {
        ...
        // 修改：使用配置的半径
        int radius = plugin.getPluginConfig().getArrivalRadius();
    ...
    private class TeleportTask extends BukkitRunnable {
        ...
        private boolean consumeResources() {
            ...
            if (!costManager.tryConsumeFuel(originBlockLoc, player)) {
                return false;
            }
...
5. 书UI一页只有2个有点少，让第一页3-4个，第二及其后面尽可能多现实（正常书的内容就可以）